...
type VethPair struct {
	veth  netlink.Link // Outer end
	vpeer netlink.Link // Inner end
}
...
func (v VethPair) Close() {
	...
	// I'm not sure if this is even supposed to be ever deleted. Vpeer is never in the host
	// network namespace, meaning that this command is never supposed to work. Additionally,
	// if I delete veth, vpeer is supposed to be deleted automatically.
	//
	// Consider this is a safety measure, if vpeer stayed in the host network namespace, and
